...
def changeColor():
    c = input("Надрукуй назву кольору, якого буде черепашка: ->") #інпут тільки для пайчарму
    t.color(c)
...
def changeBackgroundColor():
    c = input("Надрукуй назву кольору, якого буде фон: ->") #інпут тільки для пайчарму
    bgcolor(c)
...
scr.onkey(WriteText, "w")
scr.onkey(changeColor, "c")
scr.onkey(clearBackground, "x")
scr.onkey(changeBackgroundColor, "m")
