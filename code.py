...
# змінні, які відповідають за напрями переміщення м'яча
dx = 3
dy = 3
...
class Area():
    ...
    def colliderect(self, rect):  # новий метод для зіткнення
...
while not game_over:
    ball.fill() # пишемо пізніше, як Оля скаже
    platform.fill() # пишемо пізніше, як Оля скаже

    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            game_over = True
        # перевіряємо тип події і кнопку події
    ...
    # перевірка поразки та перемоги⬅️⬅️⬅️
    if ball.rect.y > 350:
    ...
    for m in monsters:
        m.draw()
        # якщо монстра торкнувся м'яч, видаляємо монстра зі списку та міняємо напрямки руху м'яча⬅️⬅️⬅️
