...
hour_counts = df_llm['hour'].value_counts().sort_index()
...
plt.scatter(hour_counts.index, hour_counts.values, color='blue', s=100)  # s - розмір точок
plt.title('Час написання запитів по годинах')
plt.xlabel('Години')
plt.ylabel('Кількість запитів')
plt.xticks(hour_counts.index)  # Показати години на осі X
plt.grid(True)
plt.show()

# Графік: Частота запитів по мовах
plt.subplot(1, 2, 1)
language_counts.plot(kind='bar', color='skyblue')
plt.title('Частота запитів по мовах')
plt.xlabel('Мова')
plt.ylabel('Кількість запитів')

# Графік: Середня кількість символів за мовами
plt.subplot(1, 2, 2)
average_text_length_by_language.plot(kind='bar', color='lightgreen')
plt.title('Середня кількість символів у текстах за мовами')
plt.xlabel('Мова')
plt.ylabel('Середня кількість символів')
...
# Завантаження даних
df_reviews = pd.read_csv('chatgpt_reviews.csv', delimiter=';')
...
# Графік: Кількість позитивних відгуків за рейтингом
plt.figure(figsize=(8, 5))
positive_reviews_count.plot(kind='bar', color='salmon')
plt.title('Кількість позитивних відгуків за рейтингом')
plt.xlabel('Рейтинг')
